...
void interpret(node *a, int *np, int N)
{
    ...
    while (command[0] != 'q')
    {
        ...
        case 'i':
        case 'I':
            hsc_insert(&k, a, np, 2, cmp_func, hash_func);
            printf("\n   Successful insert.");
            break;
        case 'd':
        case 'D':
            if (hsc_delete(&k, a, np, 2, cmp_func, hash_func) == NULL)
        ...
        case 's':
        case 'S':
            if ((p = hsc_search(&k, a, np, 2, cmp_func, hash_func)) == NULL)
